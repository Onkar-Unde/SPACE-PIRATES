...
import { database, ref, push, onValue, query, orderByChild, equalTo, get } from './firebase';
...
function SignUpForm() {
  ...
  const signup = async (e) => {
    ...
    const matchEmailQuery = query(matchEmailRef, orderByChild('email'), equalTo(email));
    ...
    try {
     ...
     // const isAdminExists = emailMatchingArray.some(user => user.userType === 'admin');
      ...
      if (userType === 'admin' && adminCount === 0) {
        await push(ref(database, 'custam_info/signup/admin'), { name, email, password, mobileNumber, userType });
        alert("Signup Successful as Admin");

        setAdminCount(1);
        clearFields();
      } else if (userType === 'user') {
        await push(ref(database, 'custam_info/signup/user'), { name, email, password, mobileNumber, userType });
  ...
  return (
    <div className="container bg">
      ...
      <p className="login-link">Already have an account? Sign In</p>
    </div>
  );
}
